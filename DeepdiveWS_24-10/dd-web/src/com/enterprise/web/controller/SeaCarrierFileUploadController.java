...
public class SeaCarrierFileUploadController implements IFileUploadController {

	@Autowired
	private FileUploadBusinessService fileUploadBusinessService;

	@RequestMapping(value = "/upload/seacarrier/save", method = RequestMethod.POST)
        ...
        if (multipartFile != null) {
            ...
            try {
            	...
            	if (fileContentAsByteArr == null || fileContentAsByteArr.length == 0)
            		...
            		fileUploadSessionBean.setRequestMapping("/upload/seacarrier/process");
	...
	@RequestMapping(value = "/upload/seacarrier/process", method = {RequestMethod.POST, RequestMethod.GET})
    public String uploadModuleProcessRequest(@ModelAttribute("fileUploadSessionBean") FileUploadSessionBean bean, Model model) {
		...
		try {
			bean = fileUploadBusinessService.processSeaCarrierFileContent(textStr, bean);
			if (!bean.isFileProcessed())
				bean.setJavascript("parent.ajax('ajaxAsyncDiv','/upload/seacarrier/processing');");
			else
	...
	@RequestMapping(value="/upload/seacarrier/form", method = {RequestMethod.POST, RequestMethod.GET})
	public String uploadModuleFormRequest(Model model) {
		return "upload/seacarrier/form";
	}
	
	@RequestMapping(value = "/upload/seacarrier/processing", method = {RequestMethod.POST, RequestMethod.GET})
    public String uploadAirportProcessingRequest(
		...
		try {
			bean = fileUploadBusinessService.processSeaCarrierFileContent(textStr, bean);
			if (!bean.isFileProcessed())
				asyncSessionObj.setJavascript("parent.ajax('ajaxAsyncDiv','/upload/seacarrier/processing');");
			else
