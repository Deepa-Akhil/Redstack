...
public class AirportFileUploadController implements IFileUploadController {
	
	@Autowired
	private FileUploadBusinessService fileUploadBusinessService;

	@RequestMapping(value = "/upload/airport/save", method = RequestMethod.POST)
        ...
        if (multipartFile != null) {
            ...
            try {
            	...
            	if (fileContentAsByteArr == null || fileContentAsByteArr.length == 0)
            		...
            		fileUploadSessionBean.setRequestMapping("/upload/airport/process");
	...
	@RequestMapping(value = "/upload/airport/process", method = {RequestMethod.POST, RequestMethod.GET})
    public String uploadModuleProcessRequest(@ModelAttribute("fileUploadSessionBean") FileUploadSessionBean bean, Model model) {
		...
		try {
			bean = fileUploadBusinessService.processAirportFileContent(textStr, bean);
			if (!bean.isFileProcessed())
				bean.setJavascript("parent.ajax('ajaxAsyncDiv','/upload/airport/processing');");
			else
	...
	@RequestMapping(value="/upload/airport/form", method = {RequestMethod.POST, RequestMethod.GET})
	public String uploadModuleFormRequest(Model model) {
		return "upload/airport/form";
	}
	
	@RequestMapping(value = "/upload/airport/processing", method = {RequestMethod.POST, RequestMethod.GET})
    public String uploadAirportProcessingRequest(
		...
		try {
			bean = fileUploadBusinessService.processAirportFileContent(textStr, bean);
			if (!bean.isFileProcessed())
				asyncSessionObj.setJavascript("parent.ajax('ajaxAsyncDiv','/upload/airport/processing');");
			else
